...
export default function CaaTable({
  ...
  function onBulkApply(value: string) {
    ...
    startTransition(async () => {
      try {
        const res = await bulkSetCaaObligationStatut(ids, selectedYear, libelle);
  ...
  // Recap transversal : TOUTES les annees avec missions + fenetre 3-ans,
  // peu importe le mode (Base ou Annee). L'utilisateur veut voir "que reste-t-il
  // a faire ?" sur tout l'historique meme quand il est focus sur un exercice.
...
function EditClientCaaModal({
  ...
  return (
    <FormModal
      title={`Modifier ${row.denomination}`}
      onClose={onClose}
      onSubmit={submit}
      submitDisabled={!denomination.trim()}
      isPending={isPending}
      error={error}
    >
